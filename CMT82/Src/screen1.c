...
//	wypisz(L); //TODO: zakomentować na koniec wypisywanie całej listy kierunkowej w terminalu
	...
	size = sprintf(data, "Ilosc wezlow: %d oraz p %d\n\r", p_max, p);
...
void screen1_action(int button) {
	...
	case 4:
//		size = sprintf(data, "p %d i p_max %d\n\r", p, p_max);
//				HAL_UART_Transmit(&huart1, data, size, 100);
		if (p < (p_max-1)) {
			p++;
			screen1_program_update();
...
void screen1_program_update() {
	...
	size = sprintf(data, "CLR 150 76 300 87 65535\n\r");
	HAL_UART_Transmit(&huart2, data, size, 1000);

	size = sprintf(data, "UF 2 %d 72 0 %s\/%s PCS\n\r", 240-(strlen(temp_pcs)+strlen(temp_pcs_done)+5)*5,  temp_pcs_done, temp_pcs);
	HAL_UART_Transmit(&huart2, data, size, 1000);

	size = sprintf(data, "CLR 220 118 260 130 65535\n\r");
	HAL_UART_Transmit(&huart2, data, size, 1000);
